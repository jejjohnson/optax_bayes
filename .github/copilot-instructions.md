...
- **Layout**: `src/` layout (`src/optax_bayes/`)
- **Testing**: pytest
...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/optax_bayes/`, so always run the commands below from the repo root.
...
uv run --group typecheck ty check src/optax_bayes
```

> **Common pitfall**: Running `ruff check src/optax_bayes/` instead of `ruff check .` misses import-sorting errors in `tests/` and `scripts/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.
