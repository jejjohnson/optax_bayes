#!/usr/bin/env bash
#
# Bootstrap the optax_bayes label taxonomy.
#
...
create "area:algorithmic" "006b75" "BLR math, Hessian estimators, primitives"
create "area:integration" "5319e7" "Ecosystem integration (gaussx, efax, equinox)"
...
# optax_bayes three-layer stack from architecture.md:
#   L0 — pure JAX primitives (BLR update math, Hessian estimators, conversions)
#   L1 — optax GradientTransformations (blr_diagonal, blr_full_rank, blr_low_rank)
#   L2 — loss wrappers, posterior extraction, schedule composition
create "layer:0-primitives" "fef2c0" "Layer 0 — pure JAX BLR primitives"
create "layer:1-components" "fbca04" "Layer 1 — optax GradientTransformations"
create "layer:2-models"     "d4c5f9" "Layer 2 — loss wrappers + posterior extraction"

echo "Creating wave:* labels..."
# optax_bayes release roadmap from boundaries.md:
#   v0.0 bootstrap → v0.1 Diagonal → v0.2 Full-Rank (gaussx) → v0.3 Low-Rank (Woodbury) → v0.4+ advanced
create "wave:0-bootstrap" "c2e0c6" "Wave 0 — repo bootstrap (v0.0)"
create "wave:1-diagonal"  "bfd4f2" "Wave 1 — BLR-Diagonal (v0.1)"
create "wave:2-fullrank"  "d4c5f9" "Wave 2 — BLR-FullRank with gaussx (v0.2)"
create "wave:3-lowrank"   "f9d0c4" "Wave 3 — BLR-LowRank via Woodbury (v0.3)"
create "wave:4-advanced"  "fef2c0" "Wave 4+ — Hutchinson, schedules, K-FAC, ecosystem showcases (v0.4+)"
